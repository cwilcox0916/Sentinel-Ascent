...
/**
 * Pooled cyan-streak projectile graphics.
 */
export class ProjectileRenderer {
  ...
  update(projectiles: ReadonlyArray<Projectile>, alpha: number): void {
    ...
    for (const p of projectiles) {
      ...
      if (!g) {
        g = new Graphics();
        g.circle(0, 0, p.radius).fill({ color: PALETTE.cyan }).stroke({ color: 0xffffff, width: 0.6, alpha: 0.8 });
        this.graphics.set(p.id, g);
        this.layers.projectiles.addChild(g);
      }
      g.position.set(
    ...
    for (const [id, g] of this.graphics) {
      if (!seen.has(id)) {
        this.layers.projectiles.removeChild(g);
        g.destroy();
